      ...
      <ul role="list" class="mx-auto max-w-7xl">
        <li class="flex justify-between gap-x-6 py-5" v-for="user in Prestatataire" :key="user.id">
          <div class="flex min-w-0 gap-x-4">
            <img class="h-12 w-12 flex-none rounded-full bg-gray-50" :src="'http://localhost:8000' + user.photo"
              alt="vgvgbv" />
                  ...
                  <div
                    ...
                    <div
                      ...
                      <div class="w-200 mb-0 mt-6 rounded-lg p-6 shadow-lg sm:p-8 lg:p-10 space-y-8">
                        ...
                        <img class="lg:h-80 md:h-64 h-48 w-full object-cover object-center"
                          :src="'http://localhost:8000' + user.photo" alt="blog">
                        <div>
                        ...
                        <div class="flex justify-center space-x-4">
                          <button
                            class="rounded-lg bg-green-500 hover:bg-green-600 px-6 py-4 text-lg font-medium text-white transition-colors"
                            type="button" @click="AccepterUser(user.id)">Accepter</button>
                          <button
                            class="rounded-lg bg-red-500 hover:bg-red-600 px-6 py-4 text-lg font-medium text-white transition-colors"
                            type="button" @click="RejeterUser(user.id)">Rejeter</button>
                        </div>
                      </div>
